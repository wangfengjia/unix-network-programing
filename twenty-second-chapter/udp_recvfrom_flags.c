...
ssize_t
udp_recvfrom_flags(int fd, void *ptr, size_t nbytes, int *flagsp, SA *sa, socklen_t *salenptr, struct unp_in_pktinfo *pktp){
...
#else
    bzero(&msg, sizeof(msg)); // make certain msg_accrightslen
#endif
    msg.msg_name = sa;
    ...
    *flagsp = msg.msg_flags;
    if (msg.msg_controllen < sizeof(struct cmsghdr) || (msg.msg_flags & MSG_TRUNC) || pktp == NULL){
        return (n);
    }
