...
void
udp_dg_echo(int sockfd, SA *pcliaddr, socklen_t clilen){
    ...
    if (setsockopt(sockfd, IPPROTO_IP, IP_RECVDSTADDR, &on, sizeof(on)) < 0){
        err_ret("setsockopt of IP_RECVADTADDR");
    }
#endif
#ifdef IP_RECVIF
    ...
    while (true){
        ...
        len = clilen;
        flags = 0;
        n = udp_recvfrom_flags(sockfd, mesg, MAXLINE, &flags, pcliaddr, &len, &pktinfo);
